...
범용 픽토그램 시안은 사진 다운로드 없이 별도 포트로 실행할 수 있습니다.

```powershell
node prototype/brc-am7/serve.mjs --port 4212 --pictogram
```

회사 PC에서 `http://127.0.0.1:4212/`를 열면 BRC-AM7 한 제품의 기존 화면에 자체 제작 PTZ 픽토그램 1개만 표시됩니다. 그림은 실물 사진이나 특정 모델의 정확한 외형을 뜻하지 않습니다. 사진 관련 기존 `content.json`은 변경하지 않고 로컬 서버가 응답을 메모리에서 바꿉니다. 픽토그램 자체의 사용 조건은 [PICTOGRAM-LICENSE.md](PICTOGRAM-LICENSE.md)에 있습니다. 다른 제품과 공개 사이트에는 적용하지 않았습니다.
